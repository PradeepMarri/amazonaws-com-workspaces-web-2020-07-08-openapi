...
func GetusersettingsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var result models.GetUserSettingsResponse
...
func CreateGetusersettingsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_userSettings_userSettingsArn",
		mcp.WithDescription("Gets user settings."),
		mcp.WithString("userSettingsArn", mcp.Required(), mcp.Description("The ARN of the user settings.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetusersettingsHandler(cfg),
	}
}
